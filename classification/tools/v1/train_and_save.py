...
from v1.preprocessing import preprocess_sequences
from v2.preprocessing_v2 import preprocess_images_v2
from v1.data import build_dataset
from v2.data_v2 import build_dataset_v2
...
def train_and_save_model(
    ...
    number_of_positive_samples=800,
    input_shape=(64, 64),
    compute_spatial_features=True,
    orientations=9,
    pixels_per_cell=(12, 16),
    cells_per_block=(4, 4),
    C=1,
    bagging=False,
    n_estimators=10,
    ...
    if version == "v1":
        ...
        # Preprocess
        features, labels = preprocess_sequences(
            sequences,
            input_shape,
            orientations,
            pixels_per_cell,
    ...
    # ----------------------------------------- #
        ...
        # Build Dataset
        imgs, labels = build_dataset_v2(
            positive_negative_ratio=positive_negative_ratio,
            number_of_positive_samples=number_of_positive_samples,
        )

        # Preprocess
        features = preprocess_images_v2(
            imgs, orientations, pixels_per_cell, cells_per_block
        )
    ...
    if bagging:
        model = BaggingClassifier(
            base_estimator=svm.SVC(gamma="scale", C=C),
    ...
    if save_config:
        ...
        (
            data["preprocessing"]["input_shape"]["y"],
            data["preprocessing"]["input_shape"]["x"],
        ) = (input_shape[0], input_shape[1])
        data["preprocessing"]["orientations"] = orientations

        data["preprocessing"]["pixels_per_cell"] = dict()
        (
            data["preprocessing"]["pixels_per_cell"]["y"],
            data["preprocessing"]["pixels_per_cell"]["x"],
        ) = (pixels_per_cell[0], pixels_per_cell[1])
        data["preprocessing"]["cells_per_block"] = dict()
        (
            data["preprocessing"]["cells_per_block"]["y"],
            data["preprocessing"]["cells_per_block"]["x"],
