...
def download_datasets(datasets):
    ...
    if "ecse415" in datasets and not os.path.exists("dataset"):
        cmds.append(
            "wget -nc -O dataset.zip https://mcgill-my.sharepoint.com/:u:/g/personal/raghav_mehta_mail_mcgill_ca/EVEvhY9_jyVEk2uSZ8wZhFYBQ58C57I7ZB55jBocKwB5Jg?download=1"
        )
...
def build_dataset_v2(
    positive_negative_ratio=1, number_of_positive_samples=2000, resize_shape=(64, 64)
):
    """
    ...
    number_of_negative_samples = int(
        (1.0 / positive_negative_ratio) * number_of_positive_samples
    )
    ...
    if samples_available < number_of_negative_samples:
        print(
            "Error: {av} positive samples available, but {req} required.".format(
                av=samples_available, req=number_of_negative_samples
            )
        )
        sys.exit(-1)
    ...
    for i in range(number_of_positive_samples):
        while True:
            ...
            random_id = vehicle_db_dict[random_db]["inds"][
                vehicle_db_dict[random_db]["i"]
            ]
            vehicle_db_dict[random_db]["i"] += 1

            filepath = (
                vehicle_db_main_path
                + random_db
                + "/"
                + os.listdir(vehicle_db_main_path + random_db)[random_id]
            )
    ...
    for db in non_vehicle_db_subpaths:
        ...
        non_vehicle_db_dict[db]["length"] = len(
            os.listdir(non_vehicle_db_main_path + db)
        )
    ...
    if samples_available < number_of_negative_samples:
        print(
            "Error: {av} negative samples available, but {req} required.".format(
                av=samples_available, req=number_of_negative_samples
            )
        )
        sys.exit(-1)
    ...
    for i in range(number_of_negative_samples):
        while True:
            ...
            if (
                non_vehicle_db_dict[random_db]["i"]
                >= non_vehicle_db_dict[random_db]["length"]
            ):
                continue

            random_id = non_vehicle_db_dict[random_db]["inds"][
                non_vehicle_db_dict[random_db]["i"]
            ]
            non_vehicle_db_dict[random_db]["i"] += 1

            filepath = (
                non_vehicle_db_main_path
                + random_db
                + "/"
                + os.listdir(non_vehicle_db_main_path + random_db)[random_id]
            )
            if not filepath.endswith(".png"):
                continue
            break
